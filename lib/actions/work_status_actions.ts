...
export async function createWorkStatus(formData: FormData) {
    try {
        ...
        const duplicateCheck = await checkDuplicateTicketNumber(ticketNumber, userId);
        if (duplicateCheck.exists) {
            return { error: 'A work status with this ticket number already exists' };
        }
...
// Update Work Status
export async function updateWorkStatus(id: string, formData: FormData, userId: string) {
    try {
        const ticketNumber = formData.get('ticketNumber') as string;

        // Check for duplicate ticket number
        const duplicateCheck = await checkDuplicateTicketNumber(ticketNumber, userId, id);
        if (duplicateCheck.exists) {
            return { error: 'Another work status with this ticket number already exists' };
        }
...
export async function getWorkStatusesGroupedByDate(userId: string) {
    try {
        ...
        const formattedGrouped: { [key: string]: any[] } = {};
...
export async function getWorkStatusesByDateRange(userId: string, startDate: Date, endDate: Date) {
    try {
        if (!userId) {
            return { error: 'User not authenticated' };
        }
...
export async function checkDuplicateTicketNumber(ticketNumber: string, userId: string, excludeId?: string) {
    try {
...
export async function getWorkStatusByTicketNumber(ticketNumber: string, userId: string) {
    try {
        if (!userId) {
            return { error: 'User not authenticated' };
        }
